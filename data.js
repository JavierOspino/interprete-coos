...
const funciones = {
  ...
  insertarFuncion: function(tipo, id, param, body) {
    if (this.validarFuncion(id)) {
      ...
      this.funcions.push(fun);
      return fun;
    } else {
      throw console.error(
        "Error - No puedes declarar dos funciones con el mismo nombre"
      );
    }
  },
  validarFuncion: function(id) {
    for (let index = 0; index < this.funcions.length; index++)
      if (this.funcions[index].id === id) return false;
  ...
  declararVariable: function(id) {
    ...
    throw console.error(
      "Error - No puedes declarar dos variables con el mismo nombre"
    );
  },
  ...
  actualizarVariable: function(id, value) {
    ...
    if (index !== -1) {
      ...
      throw console.error("Error - la variable no existe");
    }
  },
